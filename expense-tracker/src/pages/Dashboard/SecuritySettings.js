...
import { getSecuritySettings, start2FA, updateSecuritySettings, verify2FA } from "../../services/securityService";
...
function Toggle({ checked, disabled, onChange }) {
  return (
    <button
      ...
      className={`relative h-8 w-16 rounded-full transition ${checked ? "bg-cyan-400" : "bg-slate-400/40"} ${disabled ? "opacity-60" : ""}`}
    >
      <span
        ...
        <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base text-slate-400">
              The toggles below now talk directly to the backend security endpoints, so 2FA status and single-device mode stay synchronized with BE rules.
            </p>
          </div>
          ...
          <div className={`rounded-[1.75rem] border p-5 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/80"}`}>
            ...
            <div className="mt-5 space-y-3">
              ...
              <div className={`rounded-2xl p-4 ${isDark ? "bg-slate-950/60" : "bg-white"}`}>
                ...
                <p className="mt-2 text-lg font-black">{sessionMeta.expiresAt ? new Date(sessionMeta.expiresAt).toLocaleString() : "Unknown"}</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">Add an authenticator code requirement when signing in.</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-slate-400">If enabled, a new login forces older sessions to sign out.</p>
                </div>
              </div>
            ...
            <div className={`rounded-[1.5rem] border p-4 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/80"}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">{settings.is_2fa_enabled ? "Your account requires an authenticator code." : "Enable 2FA to harden the login flow."}</p>
                </div>
              </div>
            </div>
            ...
            <div className={`rounded-[1.5rem] border p-4 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/80"}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Use the logout action below if you want to terminate the current authenticated session cleanly.</p>
                </div>
              </div>
            </div>
          ...
          <div className={`w-full max-w-lg rounded-[2rem] border p-6 shadow-2xl ${isDark ? "border-white/10 bg-slate-900" : "border-white bg-white"}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-slate-400">Scan the QR code, then confirm with the six-digit code from your authenticator app.</p>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div className={`rounded-[1.5rem] border p-4 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/80"}`}>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <code className="flex-1 break-all rounded-2xl bg-slate-950/80 px-3 py-3 text-xs text-cyan-300">{twoFaSetup.secret}</code>
                    <button
                      onClick={async () => {
                ...
                <label className="block text-sm font-semibold">
                  ...
                  <input
                    ...
                    className={`w-full rounded-2xl border px-4 py-4 text-center text-2xl font-black tracking-[0.5em] ${isDark ? "border-white/10 bg-slate-950 text-white" : "border-slate-200 bg-white text-slate-900"}`}
                  />
                </label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button
                    ...
                    className={`rounded-2xl px-4 py-4 text-sm font-black ${isDark ? "bg-white/5 text-white" : "bg-slate-100 text-slate-700"}`}
                  >
                    Cancel
                  </button>
                  <button
