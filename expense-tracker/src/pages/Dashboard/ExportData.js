...
export default function ExportData() {
  ...
  const filteredTransactions = useMemo(() => {
    return filter === "all" ? transactions : transactions.filter((item) => item.type === filter);
  }, [transactions, filter]);
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base text-slate-400">
              Exports are now aligned with the backend streaming endpoints. No polling workaround, no fake async tasks, just direct Excel files generated by the BE you locked down.
            </p>
          </div>
          ...
          <div className={`rounded-[1.75rem] border p-5 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/80"}`}>
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400">The preview table below is built from the same income and expense sources used across the app.</p>
              </div>
            </div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <SummaryTile title="Income preview" value={formatCurrency(totals.income, currencyCode)} icon={TrendingUp} tone={isDark ? "border-emerald-400/15 bg-emerald-400/10" : "border-emerald-100 bg-emerald-50"} />
        <SummaryTile title="Expense preview" value={formatCurrency(totals.expense, currencyCode)} icon={TrendingDown} tone={isDark ? "border-orange-300/15 bg-orange-300/10" : "border-orange-100 bg-orange-50"} />
        <SummaryTile title="Rows selected" value={`${filteredTransactions.length}`} icon={Sparkles} tone={isDark ? "border-cyan-400/15 bg-cyan-400/10" : "border-cyan-100 bg-cyan-50"} />
      </section>
              ...
              <tbody>
                {filteredTransactions.length ? (
                  filteredTransactions.map((item) => (
                    <tr key={`${item.type}-${item.id}`} className={`border-t ${isDark ? "border-white/10 hover:bg-white/5" : "border-slate-200 hover:bg-slate-50"}`}>
