...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      setTrend(
        expenseTrend.status === "fulfilled" && Array.isArray(expenseTrend.value)
          ? expenseTrend.value
          : []
      );
      setRecentTransactions(recent.status === "fulfilled" && Array.isArray(recent.value) ? recent.value : []);
      setSystemSettings(settings.status === "fulfilled" && settings.value ? settings.value : { broadcast_message: "" });
    ...
    return (
      <div className="flex min-h-[75vh] items-center justify-center">
        <div className={`rounded-[2rem] border px-8 py-10 text-center ${isDark ? "border-white/10 bg-slate-900/70" : "border-white/80 bg-white/75"}`}>
          ...
          <p className="mt-4 text-sm font-semibold uppercase tracking-[0.3em] text-slate-400">Building your board</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <section className={`overflow-hidden rounded-[2.25rem] border p-6 shadow-2xl ${isDark ? "border-white/10 bg-slate-900/70" : "border-white/80 bg-white/75"}`}>
        <div className="grid gap-6 xl:grid-cols-[1.4fr_0.9fr]">
          <div>
            ...
            <p className="mt-4 max-w-2xl text-base text-slate-400">
              Your frontend is now aligned with backend metrics, security settings, exports, and analytics. Everything on this board is reading the live BE contract.
            </p>

            <div className="mt-6 flex flex-wrap gap-3">
              <Link to="/income" className="inline-flex items-center gap-2 rounded-2xl bg-cyan-400 px-5 py-3 text-sm font-bold text-slate-950 transition hover:translate-y-[-1px]">
                <Plus size={16} />
                Add income
              </Link>
              <Link to="/expense" className="inline-flex items-center gap-2 rounded-2xl border border-orange-300/30 bg-orange-300/10 px-5 py-3 text-sm font-bold text-orange-200 transition hover:translate-y-[-1px]">
                <Plus size={16} />
                Add expense
              </Link>
              <Link to="/analytics" className={`inline-flex items-center gap-2 rounded-2xl px-5 py-3 text-sm font-bold transition hover:translate-y-[-1px] ${isDark ? "bg-white/5 text-white" : "bg-slate-100 text-slate-700"}`}>
                <Layers3 size={16} />
                Open analytics
              </Link>
            </div>
          ...
          <div className={`rounded-[2rem] border p-5 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/70"}`}>
            <div className="flex items-center justify-between">
              ...
              <div className={`rounded-2xl px-4 py-2 text-sm font-bold ${balance >= 0 ? "bg-emerald-400/15 text-emerald-300" : "bg-rose-400/15 text-rose-300"}`}>
            ...
            <div className="mt-6 space-y-4">
              <div>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold">{budget > 0 ? formatCurrency(budget, currencyCode) : "Not set"}</span>
                </div>
                <div className={`mt-3 h-3 overflow-hidden rounded-full ${isDark ? "bg-slate-800" : "bg-slate-200"}`}>
                  <div
                    className={`h-full rounded-full ${budgetUsage >= 100 ? "bg-rose-400" : budgetUsage >= 80 ? "bg-orange-300" : "bg-cyan-400"}`}
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className={`rounded-2xl p-4 ${isDark ? "bg-slate-950/60" : "bg-white"}`}>
                  ...
                  <p className="mt-2 text-lg font-black">{chartData.length ? formatLongDate(chartData[chartData.length - 1].rawDate) : "No data"}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        {systemSettings?.broadcast_message && (
          <div className={`mt-6 flex items-center gap-3 rounded-[1.5rem] border px-4 py-4 ${isDark ? "border-orange-300/20 bg-orange-300/10 text-orange-100" : "border-orange-200 bg-orange-50 text-orange-700"}`}>
            <BellRing size={18} />
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricCard
          ...
          subtitle="Backend-synced spending across all categories."
          icon={ArrowDownRight}
          tone={{
      ...
      <section className={`rounded-[2rem] border p-5 shadow-xl ${isDark ? "border-white/10 bg-slate-900/70" : "border-white/80 bg-white/75"}`}>
        <div className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <Link to="/analytics" className={`inline-flex items-center gap-2 rounded-2xl px-4 py-3 text-sm font-bold ${isDark ? "bg-white/5 text-white" : "bg-slate-100 text-slate-700"}`}>
            <Sparkles size={16} />
          ...
          {recentTransactions.length ? (
            recentTransactions.map((transaction) => {
              ...
              return (
                <div
                  key={transaction.id}
                  className={`flex flex-col gap-4 rounded-[1.5rem] border px-4 py-4 sm:flex-row sm:items-center sm:justify-between ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/70"}`}
                >
                  <div className="flex items-center gap-4">
                    <div className={`flex h-12 w-12 items-center justify-center rounded-2xl text-xl ${type === "income" ? "bg-emerald-400/15" : "bg-orange-300/15"}`}>
                      {transaction.emoji || (type === "income" ? "💰" : "💸")}
                    </div>
                    <div>
                  ...
                  <div className="flex items-center justify-between gap-4 sm:justify-end">
                    ...
                    <div className={`text-right text-lg font-black ${type === "income" ? "text-emerald-300" : "text-orange-200"}`}>
                      {type === "income" ? "+" : "-"} {formatCurrency(transaction.amount, transaction.currency_code || currencyCode)}
                    </div>
                  </div>
                </div>
              );
            ...
            <div className={`rounded-[1.5rem] border border-dashed px-6 py-14 text-center ${isDark ? "border-white/10 text-slate-400" : "border-slate-200 text-slate-500"}`}>
              ...
              <p className="mt-4 text-sm">No recent activity yet. Your latest backend transactions will appear here.</p>
            </div>
          )}
        </div>
      </section>
