...
export default function AnalyticsUnified() {
  ...
  const transactionRows = useMemo(
    () => (Array.isArray(analytics?.transactions) ? analytics.transactions : []),
    [analytics]
  );
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-base text-slate-400">
              Filters, totals, charts, and transaction rows on this screen now come from the backend analytics summary
              endpoint instead of local approximations.
            </p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <InsightCard
          ...
          helper={
            analytics?.most_expensive_day
              ? `Most expensive day: ${formatLongDate(analytics.most_expensive_day)}`
              : "Most expensive day will appear here."
          }
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Filtered total currently sits at {formatCurrency(analytics?.total_income, currencyCode)}.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className={`rounded-[1.5rem] border p-4 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/80"}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    Average daily spending is {formatCurrency(analytics?.average_daily_spending, currencyCode)}.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className={`rounded-[1.5rem] border p-4 ${isDark ? "border-white/10 bg-white/5" : "border-slate-200 bg-slate-50/80"}`}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">
                    {analytics?.most_expensive_day
                      ? formatLongDate(analytics.most_expensive_day)
                      : "No peak day available for this range."}
                  </p>
                </div>
              </div>
            </div>
            ...
            <tbody>
              {transactionRows.length ? (
                transactionRows.map((transaction) => {
                  const badge = transaction.emoji || (transaction.type === "income" ? "$" : "-");
