...
export default function Sidebar({ collapsed, setCollapsed, theme, setTheme, isMobile }) {
  ...
  const renderMenuItem = (item) => {
    ...
    return (
      <div key={item.path} id={tourId} className="relative group w-full px-2 mb-1">
        <Link
          to={item.path}
          className={`flex items-center gap-4 px-4 py-3 rounded-xl w-full transition-all duration-300 ease-out
            ${active ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/30 translate-x-1" : theme === "dark" ? "text-gray-400 hover:bg-white/10 hover:text-white hover:translate-x-1" : "text-gray-500 hover:bg-blue-50 hover:text-blue-600 hover:translate-x-1"}
            ${collapsed ? "justify-center px-0" : ""}`}
        >
          <div className={`${active ? "animate-pulse-slow" : ""}`}>{item.icon}</div>
          {!collapsed && <span className="text-sm font-semibold tracking-wide truncate">{item.name}</span>}
        </Link>
        {collapsed && (
  ...
  return (
    <div id="tour-sidebar" className={`sticky top-0 h-screen py-3 pl-3 transition-[width] duration-300 z-50 ${collapsed ? "w-24" : "w-72"}`}>
        <aside
            onMouseEnter={!isMobile ? () => setCollapsed(false) : undefined}
            onMouseLeave={!isMobile ? () => setCollapsed(true) : undefined}
            className={`h-full flex flex-col justify-between rounded-3xl border shadow-2xl backdrop-blur-xl overflow-hidden transition-colors duration-300 ${theme === "dark" ? "bg-gray-900/95 border-white/10 shadow-black/50" : "bg-white/90 border-white/40 shadow-blue-100"}`}
        >
                ...
                <div className="flex flex-col items-center justify-center pt-8 pb-4 px-4"> 
                    <div className={`flex items-center gap-3 mb-6 ${collapsed ? "flex-col" : "flex-row"}`}>
                        ...
                        <div className="flex flex-col">
                            <h2 className="text-xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-blue-500 to-purple-500">Expense Tracker</h2>
                        </div>
                        )}
                    </div>
                    {user.is_admin && !collapsed && (
                        <div className={`flex p-1 rounded-xl w-full mb-2 ${theme === 'dark' ? 'bg-black/40' : 'bg-gray-100'}`}>
                            <button onClick={() => { setViewMode('personal'); navigate('/dashboard'); }} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all duration-300 ${viewMode === 'personal' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}><User size={14} /> Personal</button>
                            <button onClick={() => { setViewMode('admin'); navigate('/admin/dashboard'); }} className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all duration-300 ${viewMode === 'admin' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'}`}><ShieldCheck size={14} /> Admin</button>
                        </div>
                    )}
                     {user.is_admin && collapsed && (
                        <div className="mb-4 flex flex-col gap-2">
                             <button onClick={() => { setViewMode('personal'); navigate('/dashboard'); }} className={`p-2 rounded-lg ${viewMode === 'personal' ? 'bg-blue-600 text-white' : 'text-gray-500'}`}><User size={16} /></button>
                             <button onClick={() => { setViewMode('admin'); navigate('/admin/dashboard'); }} className={`p-2 rounded-lg ${viewMode === 'admin' ? 'bg-purple-600 text-white' : 'text-gray-500'}`}><ShieldCheck size={16} /></button>
                        </div>
                     )}
                </div>
                <nav className="flex flex-col gap-1 px-2 flex-1 pb-20">
                    {(viewMode === 'admin' ? adminMenuStructure : personalMenu).map((section, index) => (
                        <div key={index} className="mb-4">
                            {!collapsed && <h3 className="px-4 mb-2 text-[10px] font-bold uppercase tracking-widest text-gray-400 dark:text-gray-600">{section.category}</h3>}
                ...
                <div className={`rounded-2xl p-3 transition-all duration-300 group ${theme === "dark" ? "hover:bg-white/5" : "hover:bg-white hover:shadow-md"}`}>
                    <div onClick={(e) => { e.stopPropagation(); navigate("/profile"); }} className={`flex items-center gap-3 cursor-pointer ${collapsed ? "justify-center" : ""}`}>
                        ...
                        <div className="flex-1 overflow-hidden">
                            ...
                            <p className={`text-[10px] font-bold uppercase ${user.is_admin ? 'text-purple-500' : 'text-gray-500'}`}>{user.is_admin ? "Administrator" : "Member"}</p>
                        </div>
                        )}
                    </div>
                    {!collapsed && (
                        <div className="grid grid-cols-2 gap-2 mt-3">
                            <button onClick={(e) => { e.stopPropagation(); setTheme(theme === "dark" ? "light" : "dark"); }} className={`flex items-center justify-center py-2 rounded-lg transition-colors ${theme === "dark" ? "bg-gray-800 hover:bg-gray-700 text-yellow-400" : "bg-white border border-gray-200 hover:bg-gray-100 text-gray-600"}`} title="Toggle Theme">{theme === "dark" ? <Sun size={16} /> : <Moon size={16} />}</button>
                            <button onClick={(e) => { e.stopPropagation(); handleLogout(); }} className="flex items-center justify-center py-2 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-500 transition-colors" title="Logout"><LogOut size={16} /></button>
                        </div>
                    )}
                </div>
            </div>
