...
export default function FinBotWidget() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // ✅ PHÁT TÍN HIỆU CẬP NHẬT (QUAN TRỌNG)
      if (botReply.includes("[REFRESH]")) {
          console.log("♻️ FinBot: Phát tín hiệu cập nhật dữ liệu...");
          botReply = botReply.replace("[REFRESH]", "").trim();
          // Phát sự kiện để Income.jsx và Home.jsx nghe thấy
      ...
      setMessages((prev) => [...prev, { role: "bot", content: "⚠️ Mất kết nối Server. Hãy kiểm tra lại Backend!" }]);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // ... (Phần render giữ nguyên như code cũ của bạn)
  return (
        ...
        <div className="pointer-events-auto w-[380px] h-[600px] bg-white dark:bg-gray-800 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 flex flex-col mb-4 overflow-hidden animate-in slide-in-from-bottom-5 duration-300">
           <div className="bg-gradient-to-r from-blue-600 to-purple-600 p-4 flex justify-between items-center text-white">
            <div className="flex items-center gap-2">
              ...
              <div><h3 className="font-bold text-sm">FinBot AI</h3></div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar bg-gray-50 dark:bg-gray-900">
            {messages.map((msg, idx) => (
              <div key={idx} className={`flex flex-col ${msg.role === "user" ? "items-end" : "items-start"}`}>
                <div className={`max-w-[85%] p-3 rounded-2xl text-sm shadow-sm ${msg.role === "user" ? "bg-blue-600 text-white rounded-tr-none" : "bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded-tl-none border border-gray-200 dark:border-gray-600"}`}>
                  <ReactMarkdown>{msg.content}</ReactMarkdown>
                </div>
                {msg.chart && (
                    <div className="mt-2 w-full max-w-[90%] bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-600 shadow-md">
                        <div className="h-40 w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <PieChart>
                                    <Pie data={msg.chart.data} innerRadius={35} outerRadius={55} paddingAngle={5} dataKey="value">
                                        {msg.chart.data.map((entry, index) => <Cell key={`cell-${index}`} fill={CHART_COLORS[index % CHART_COLORS.length]} />)}
                                    </Pie>
                                    <Tooltip />
                                </PieChart>
                            </ResponsiveContainer>
          ...
          <div className="p-3 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex gap-2">
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyPress} placeholder="Nhập..." className="flex-1 bg-gray-100 dark:bg-gray-700 text-gray-900 dark:text-white px-4 py-2 rounded-full text-sm outline-none" />
            <button onClick={handleSend} disabled={isLoading} className="p-2 bg-blue-600 text-white rounded-full"><Send size={18} /></button>
          </div>
        </div>
      )}
